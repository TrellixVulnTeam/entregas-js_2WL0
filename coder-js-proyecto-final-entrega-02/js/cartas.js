...
function jugarCartaJugador(pathParam){
    ...
    for ( x = 0 ; x < manoJugador.length ; x++ ){
      ...
      if (nombreArchivoEnPath == manoJugador[x].path){
        ...
        // (indice 0 ya que lo que devuelve el splice 
        // es un array, en este caso de 1 elemento)
        mesaJugador.push(cartaJugada[0]);

      }
    }

    // muestra vectores modificados en consola
    ...
    // ** Cambio de turno ** indica que es el turno de la PC
    localStorage.setItem('turnoActual', 'PC');

    // ** Cambio de mano **
    // se fija quien empieza, para cambiar la mano o no

    // si ya jugó la PC (es mano)
    if (localStorage.getItem('quienEsMano') == 'PC'){

      // revisa que mano se esta jugando
      ...
      // guarda el nuevo valor de manoActual en el storage
